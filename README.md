...
```json
{
  ...
  "input_files": {
    ...
    "building_register_csv": "data/building_register/building_register.csv",
...
`resolution_m`은 지면 격자와 건물 벽면·지붕 수음점에 공통으로
...
건물과 토지피복도는 계산 영역 전체, 등고선은 계산 영역 외곽 800m까지
포함해야 합니다. 범위가 부족하면 데이터 범위, 필요 범위, 방향별 부족 거리를
출력하고 계산을 시작하지 않습니다. 토지피복도는 지면 격자별 실제 피복률도
검사하며, 등고선 고도 보간이 하나라도 실패하면 지면 수음점 저장을 중단합니다.
...
- `scripts/generate_terrain_receivers.py`: 지면 수음점 및 지면계수 생성
